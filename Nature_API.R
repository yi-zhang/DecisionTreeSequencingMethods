### Mapping Journal Impact Factor to Extracted Publications
...
test<-data.frame()
test<-json2
test<-data.frame(lapply(my_journals[,2], function(v){if (is.character(v)) return(toupper(v)) else return(v)}))
